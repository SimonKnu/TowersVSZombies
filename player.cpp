...
bool Player::checkCollisionBorder(int maxX, int maxY){
    if ((this->getPosition().x-16)<0 || (this->getPosition().y-16)<0 || (this->getPosition().x+16) > maxX || (this->getPosition().y+16) >maxY){
        return true;
    }else{
        return false;
    }
