...
void MainWindow::start(sf::RenderWindow* window){
    ...
    Menu menuDefeat("Defeat !",400,200);
    Menu menuEndWave("",300,300);
    Menu menuEndMoney("",500,300);
    ...
    while(window->isOpen()){
        ...
        if(true == launchGame){
            ...
            if (!endGame){
                ...
                elapsedEnd = clock.getElapsedTime();

                std::stringstream ss;
                ss<<this->player->getMoney()<<"$";
                menuEndMoney.changerText(ss.str());
